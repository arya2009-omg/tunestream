(function(){
  ...
  window.TuneStreamCloud={
    ...
    async createBlend(name="Blend"){const u=await this.session();if(!u)throw new Error("Please login");const r=await client.from("blends").insert({name,created_by:u.user.id}).select().single();if(r.error)throw r.error;await client.from("blend_members").insert({blend_id:r.data.id,user_id:u.user.id});return r.data},

(function(){
  ...
  window.TuneStreamCloud={
    ...
    async unreadNotifications(){const u=await this.session();if(!u)return 0;const r=await client.from("notifications").select("id",{count:"exact",head:true}).eq("user_id",u.user.id).eq("read",false);if(r.error)throw r.error;return r.count||0},
    ...
    async addFriend(userId){const u=await this.session();if(!u)throw new Error("Please login");if(userId===u.user.id)throw new Error("You cannot add yourself");const r=await client.from("friendships").insert({requester_id:u.user.id,addressee_id:userId}).select().single();if(r.error)throw r.error;await client.from("notifications").insert({user_id:userId,actor_id:u.user.id,type:"friend_request",message:"sent you a friend request",entity_id:r.data.id});return r.data},
  ...
  async friends(){const u=await this.session();if(!u)throw new Error("Please login");const r=await client.from("friendships").select("id,requester_id,addressee_id,status,created_at,requester:profiles!friendships_requester_id_fkey(id,username,display_name),addressee:profiles!friendships_addressee_id_fkey(id,username,display_name)").or("requester_id.eq."+u.user.id+",addressee_id.eq."+u.user.id).order("created_at",{ascending:false});if(r.error)throw r.error;return r.data||[]},
    async respondFriend(id,status){const u=await this.session();if(!u)throw new Error("Please login");if(!["accepted","blocked"].includes(status))throw new Error("Invalid status");const r=await client.from("friendships").update({status}).eq("id",id).eq("addressee_id",u.user.id).select().single();if(r.error)throw r.error;const target=r.data.requester_id;if(status==="accepted")await client.from("notifications").insert({user_id:target,actor_id:u.user.id,type:"friend_accepted",message:"accepted your friend request",entity_id:id});return r.data},

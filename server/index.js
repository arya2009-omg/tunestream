...
app.get("/api/music/audius/search",async(req,res)=>{\n  const key=String(process.env.AUDIUS_API_KEY||"").trim();\n  if(!key)return res.status(503).json({message:"Audius API is not configured on the server yet"});\n  const q=String(req.query.q||"").trim();\n  if(!q)return res.json([]);\n  const limit=Math.min(Math.max(Number(req.query.limit||10),1),25);\n  try{\n    const u="https://api.audius.co/v1/tracks/search?query="+encodeURIComponent(q)+"&limit="+limit+"&sortMethod=relevant";\n    const r=await fetch(u,{headers:{"x-api-key":key,accept:"application/json"}});\n    if(!r.ok)return res.status(r.status).json({message:"Audius request failed"});\n    const j=await r.json();\n    const tracks=(j.data||[]).filter(t=>t.is_streamable!==false).map(t=>({\n      id:"audius-"+t.id,title:t.title||"Unknown",artist:t.user?.name||t.user?.handle||"Audius artist",\n      album:t.albumTitle||"Audius",emoji:"🎧",genre:t.genre||"Music",\n      audio:"https://api.audius.co/v1/tracks/"+encodeURIComponent(t.id)+"/stream?app_name=TuneStream",\n      cover:t.artwork?.["480x480"]||t.artwork?.["1000x1000"]||t.artwork?.["150x150"]||null,\n      source:"Audius",sourceUrl:"https://audius.co/"+(t.user?.handle||"")+"/"+(t.permalink||"")\n    }));\n    res.json(tracks);\n  }catch(e){console.error("Audius search error",e);res.status(502).json({message:"Audius service unavailable"})}\n});\napp.get("/api/songs",(req,res)=>{const songs=read("songs.json").sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));res.json(songs.map(cleanSong))});
